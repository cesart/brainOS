      ...
      <div className={`flex flex-row flex-nowrap overflow-x-auto items-center md:flex-col md:overflow-visible md:items-end gap-2 px-2 md:py-4 flex-shrink-0 ${kbVisible ? "py-1" : "py-2"}`}>
        {tools.map((tool) => (
          <button
            ...
            onClick={tool.action}
            title={tool.label}
            className="flex items-center justify-center p-2 rounded-md text-muted-foreground border border-border hover:text-foreground hover:bg-accent transition-colors"
          >
            {tool.icon}
          </button>
        ))}
